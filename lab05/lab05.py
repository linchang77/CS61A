def berry_finder(t):
    ...
    if label(t)=='berry':
        ...
        for childtree in branches(t):
            if berry_finder(childtree):
                return True
    return False
...
def distance(city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sqrt((get_lat(city_a)-get_lat(city_b))*(get_lat(city_a)-get_lat(city_b))+(get_lon(city_b)-get_lon(city_a))*(get_lon(city_b)-get_lon(city_a)))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    distance1=distance(city_a,make_city('d',lat,lon))
    distance2=distance(city_b,make_city('d',lat,lon))
...
def dejavu(t, n):
    ...
    if n==label(t):
        ...
        return all([ dejavu(b,n-label(t)) for b in branches(t)])
