...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    truelist=[v for v in paragraphs if select(v)]
    if len(truelist)>k:
        return truelist[k]
    else:
        return ''
...
def about(subject):
    ...
    "*** YOUR CODE HERE ***"
    def select(s):
        words = set(split(lower(remove_punctuation(s))))
        for x in subject:
            if x in words:
                return True
        return False   
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for x in word_list:
        if x==typed_word:
            return x
    accuracy={k:diff_function(typed_word,k,limit)  for k in word_list}
    minlimit=min(list(accuracy.values()))
    if minlimit>limit:
        return typed_word
    else:
        for x in list(accuracy):
            if accuracy[x]==minlimit:
                return x
    # END PROBLEM 5
...
def minimum_mewtations(typed, source, limit):
    ...
    if len(typed) == 0 or len(source) == 0: # Feel free to remove or add additional cases
        ...
        add = 1+minimum_mewtations(source[0]+typed, source, limit-1) # ng -> ing # Fill in these lines
        remove = 1+minimum_mewtations(typed[1:], source, limit-1) # ckiteus -> kiteus
        substitute = 1+minimum_mewtations(source[0]+typed[1:], source, limit-1) # cats -> mats
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def time_per_word(words, timestamps_per_player):
    ...
    for player in timestamps_per_player:
        ...
        for i in range(1,len(player)):
            pre_time+=[player[i]-player[i-1]]
        time+=[pre_time]
    return match(words,time[1:])
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for i in range(len(get_all_times(match))-1):
        answer+=[[]]
    for i in range(len(get_all_words(match))):
        min=0
        for p in range(len(get_all_times(match))):
            if time[p][i]<time[min][i]:
                min=p
        # get the min player number p
        answer[min]+=[words[i]]
    return answer
    # END PROBLEM 10
